...
public class RestRegistryGenerator extends BaseGenerator<Collection<ClassDocInfo>> {
    ...
    @Override
    public void generate(Collection<ClassDocInfo> controllers) {
    ...
    private void generateRestRegistryMap(Collection<ClassDocInfo> controllers) {
        ClassDocInfo tmp = new ClassDocInfo();
        ...
        for (ClassDocInfo c : controllers) {
            ...
            for (List<MethodDocInfo> methods : s.getMethodsOnName().values()) {
                for (MethodDocInfo method : methods) {
